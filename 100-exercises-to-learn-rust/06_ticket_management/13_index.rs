...
impl Index<TicketId> for TicketStore {
    type Output = Ticket;
    fn index(&self, index: TicketId) -> &Self::Output { self.get(index).unwrap() }
}
impl Index<&TicketId> for TicketStore {
    type Output = Ticket;
    fn index(&self, index: &TicketId) -> &Self::Output { &self[*index] }
}
...
mod tests {
    ...
    #[test] fn works() {
        ...
        let id1 = store.add_ticket(TicketDraft { title: ticket_title(), description: ticket_description() });
        let _ticket1 = &store[id1];
        let id2 = store.add_ticket(TicketDraft { title: ticket_title(), description: ticket_description() });
