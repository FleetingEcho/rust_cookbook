...
impl TicketStoreClient {
    pub fn insert(&self, draft: String) -> u64 {
        ...
        self.sender.send(Command::Insert { draft, response_channel: resp_s }).unwrap();
        resp_r.recv().unwrap()
    }
    pub fn get(&self, id: u64) -> Option<String> {
        let (resp_s, resp_r) = channel();
        self.sender.send(Command::Get { id, response_channel: resp_s }).unwrap();
        resp_r.recv().unwrap()
    }
}
