...
fn main() {
    ...
    for x in &arr {           // TS: for (const x of arr)
        print!("{x} ");
    }
    println!();

    // 带索引遍历
    for (i, x) in arr.iter().enumerate() { // TS: arr.forEach((x, i) => ...)
        print!("[{i}]={x} ");
    }
    println!();
    ...
    // --- join ---
    let joined = a.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", ");
    println!("join: {joined}");            // TS: join(", ")

    // --- flat_map（类似 TS flatMap）---
    let nested_words = vec!["hello world", "rust is great"];
    let words: Vec<&str> = nested_words.iter().flat_map(|s| s.split_whitespace()).collect();
    ...
    println!("count 偶数: {}", data.iter().filter(|&&x| x % 2 == 0).count());
