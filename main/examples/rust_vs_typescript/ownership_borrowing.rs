...
fn main() {
    ...
    // 六、可变借用（&mut）
    // TS 没有区分可变/不可变引用的概念
    // Rust：可变借用同时只能有一个，不能与不可变借用共存
    // ============================================================

    fn change(s: &mut String) {
        s.push_str("，世界"); // 通过可变借用修改值
    }
    ...
    println!("├────────────────────────────┼────────────────────────────────────┤");
    println!("│ GC 自动管理内存            │ 所有权系统编译期验证               │");
    println!("│ 赋值对象=引用拷贝          │ 赋值=移动（Move）或显式 clone()    │");
    println!("│ 函数参数始终是引用         │ 要么转移所有权，要么显式 & 借用    │");
    println!("│ 无可变/不可变区分          │ &（多读）vs &mut（一写）严格区分    │");
    println!("│ 可以同时有多个引用         │ 多 & 或一 &mut，不能共存           │");
    println!("│ 浅拷贝 {{...obj}}           │ 需 clone()（深拷贝，性能可预期）   │");
    println!("│ 悬垂引用不可能（GC）       │ 编译器防止悬垂引用                 │");
    println!("│ 无借用检查                 │ 借用检查器在编译期验证引用安全     │");
    println!("└────────────────────────────┴────────────────────────────────────┘");
}
