// #[derive(...)] 让编译器自动生成 trait 实现。
// 这里集中演示每个常用 derive 的作用和限制。
...
// ── Clone 和 Copy ─────────────────────────────────────────────────────────────
// Clone：显式深拷贝，调用 .clone()。
// Copy ：隐式按位拷贝，赋值/传参时自动复制，不会发生移动。
//        Copy 要求类型里的所有字段也都是 Copy（不能含 String、Vec 等）。
...
// Eq：在 PartialEq 之上声明"等价关系是完全的"（所有值都能比较）。
//     整数、字符串等实现了 Eq；f64 没有。
...
// Ord       ：全序，能用 .sort()、.max()、.min()。
// 派生 Ord 要先派生 PartialOrd、Eq、PartialEq。
// 派生时按字段声明顺序比较（先 major，再 minor，再 patch）。
...
// ── Hash ──────────────────────────────────────────────────────────────────────
// 让类型可以作为 HashMap / HashSet 的键。
// 要求：实现了 Hash 的类型必须同时实现 Eq。
// 规则：a == b 必须推出 hash(a) == hash(b)，否则会有 bug。
...
// ── Default ───────────────────────────────────────────────────────────────────
// 生成 Type::default()，每个字段使用其自身的 Default 值
//（数字→0，bool→false，String→""，Vec→[]，Option→None）。
...
// ── 不能自动 derive 的情况 ────────────────────────────────────────────────────
// 如果某个字段的类型没有实现目标 trait，derive 会编译报错。
// 例如：含 *mut T 的结构体无法 derive Clone（裸指针不是 Clone）。
// 解决方法：手动实现 trait，或换用安全的包装类型。
