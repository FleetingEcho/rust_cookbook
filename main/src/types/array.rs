...
// | 特性 | Rust | TypeScript |
// |------|------|-----------|
// | 数组类型 | `[T; N]` — 定长（编译期已知） | `T[]` 或 `Array<T>` — 动态长度 |
// | 动态数组 | `Vec<T>` | `Array<T>` |
// | 越界访问 | 运行时 panic！ | 返回 `undefined` |
// | 遍历 | `for i in &arr` 或迭代器 | `for...of` 或 `.forEach()` |
// | 切片 | `&arr[0..3]`（引用，不拷贝） | `.slice(0,3)`（返回新数组） |
//
// ⚠️ TS 没有"定长数组"的概念——所有数组都是动态的。
//    Rust 的 `[T; N]` 是编译期大小固定的栈上数据，性能更高。
//
