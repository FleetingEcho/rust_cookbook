...
**TS 没有单独的 `char` 类型；Rust 的 `char` 是 Unicode 标量值（4 字节）。**
...
| TypeScript | Rust |
|---|---|
| `number`（64位浮点） | `i8`/`i16`/`i32`/`i64`/`u8`/`u16`/`u32`/`u64`/`f32`/`f64` |
| `boolean` | `bool` |
| `string` | `String`（堆）或 `&str`（借用） |
| 无 `char` 类型 | `char`（4字节 Unicode 标量） |
| `string[0]` 取字符 | `.chars().nth(i)` 安全取字符 |
| `parseInt`/`parseFloat` | `.parse::<i32>()` / `.parse::<f64>()` |
| `Math.floor`/`ceil`/`round` | `.floor()`/`.ceil()`/`.round()` |
| `Number.MAX_SAFE_INTEGER` | `i32::MIN`, `u8::MIN` 等 |
| 整数除法返回浮点 | 整数除法截断（用 f64 得浮点结果） |
