...
| 智能指针 | 用途 |
|---|---|
| `Box<T>` | 单一所有者，堆分配，递归类型，trait 对象 |
| `Rc<T>` | 单线程多所有者，不可变共享 |
| `RefCell<T>` | 运行时借用检查，单线程内部可变性 |
| `Rc<RefCell<T>>` | 单线程共享可变（TS 的默认行为） |
| `Arc<T>` | 多线程多所有者，不可变共享 |
| `Arc<Mutex<T>>` | 多线程共享可变（最安全的多线程共享） |

> **TS 对比**：GC 自动处理所有这些场景，代价是运行时开销。Rust 手动选择合适的指针，换取零运行时 GC 开销。
