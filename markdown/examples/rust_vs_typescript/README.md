...
| # | 文件 | 核心概念 | 与 TS 的关键差异 |
|---|------|---------|----------------|
| 8 | [所有权与借用](./ownership_borrowing.md) | Move、`&`、`&mut`、`Clone` | ⚠️ Rust 独有；多读或一写的严格规则 |
| 9 | [结构体](./structs.md) | `struct`、`impl`、`#[derive]`、泛型结构体 | `struct` + `impl` 代替 `class`；`Option<T>` 代替 `?:` |
| 10 | [枚举](./enums.md) | `enum` 带数据、`Option`/`Result` 源码 | ⚠️ Rust 王牌特性，TS 判别联合的增强版 |
| 11 | [模式匹配](./pattern_matching.md) | `match`、守卫、`@` 绑定、`if let`、`let else` | 编译器强制穷举；`match` 是表达式 |
| 12 | [控制流](./control_flow.md) | `if`、`loop`、`while`、`for` | `if` 是表达式；`loop` 可返回值 |

### 🟠 第三阶段：类型系统深入

| # | 文件 | 核心概念 | 与 TS 的关键差异 |
|---|------|---------|----------------|
| 13 | [Option 与 Result](./option_result.md) | `Some`/`None`、`Ok`/`Err`、`?` 运算符、combinators | 代替 `null`/`undefined` 和 `try/catch` |
| 14 | [高级错误处理](./error_handling_advanced.md) | 自定义错误类型、`From` trait、`thiserror`、`anyhow` | 枚举错误 vs `extends Error` |
| 15 | [Trait](./traits.md) | trait 定义/实现、默认方法、`dyn`、标准库 trait | 代替 `interface` + `abstract class` |
| 16 | [泛型](./generics.md) | 泛型函数/结构体、trait bound、关联类型、const 泛型 | 单态化零开销 vs 运行时擦除 |

### 🔴 第四阶段：Rust 独有概念（TS 完全没有）

| # | 文件 | 核心概念 | 与 TS 的关键差异 |
|---|------|---------|----------------|
| 17 | [生命周期](./lifetimes.md) | `'a`、省略规则、`'static` | ⚠️ 编译期验证引用安全，零运行时开销 |
| 18 | [RAII 与 Drop](./raii_drop.md) | `Drop` trait、资源自动释放、作用域守卫 | `drop()` vs `try/finally`；编译器确保释放 |
| 19 | [智能指针](./smart_pointers.md) | `Box`、`Rc`、`RefCell`、`Arc`、`Mutex` | 代替 GC；按场景选择合适指针 |
| 20 | [闭包与迭代器](./closures_iter.md) | `Fn`/`FnMut`/`FnOnce`、迭代器链、`map`/`filter`/`fold` | 三大闭包 trait vs 统一箭头函数；惰性求值 |
