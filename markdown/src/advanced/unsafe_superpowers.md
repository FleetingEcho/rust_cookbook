...
- 让 Rust 代码可以被 C 语言动态库调用（比如 `lib.so`、`dll`）。
- 需要搭配 `cargo build --release --crate-type=cdylib` 生成共享库。

## 6. 安全封装 unsafe 代码

为了减少 `unsafe` 代码的使用范围，可以用**安全抽象**包装 `unsafe` 代码。例如 `split_at_mut` 示例：
...
详细对照 → [rust_vs_typescript.rs §22 "unsafe"](../rust_vs_typescript.rs)
