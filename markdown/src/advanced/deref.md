...
在 Rust 里，智能指针之所以智能，是因为它们不仅仅是普通的指针，还实现了 `Deref` 和 `Drop` 这两个特征：
...
这里 `String` 实现了 `Deref<Target=str>`，所以 `&String` 可以自动转换为 `&str`。
...
1. `MyBox<String>` 先 `Deref` 为 `&String`
2. `String` 再 `Deref` 为 `&str`
3. 最终 `&MyBox<String>` 自动变成 `&str`
...
1. `&mut MyBox<String>` 先 `DerefMut` 成 `&mut String`
2. `&mut String` 传递给 `display` 函数
