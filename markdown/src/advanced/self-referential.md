...
**优点：** 避免生命周期问题，可以安全访问 value。清晰易懂，避免 Rust 严格的借用规则。

**缺点：** 需要 unsafe，使用不当可能导致未定义行为（UB）。
...
**优点：** Pin<Box<Self>> 确保结构体不会被移动，避免悬空指针问题。NonNull<T> 代替裸指针，避免 null 访问。
...
**优点：** 直接支持自引用，不需要 unsafe。代码更简洁，可读性更好。

**缺点：** 受限于 ouroboros 规则，不能随意修改数据。
...
**优点：** 适用于复杂数据结构。Rc<RefCell<T>> 允许多个可变引用，避免 Rust 严格的借用规则。

**缺点：** 运行时开销，每次访问都需要借用检查。

## 总结

- Rust 不支持自引用结构体，因为所有权规则不允许。
- 可选方案：Option、unsafe 裸指针、Pin、ouroboros 库、Rc + RefCell。
- 每种方案有其适用场景和局限性。
