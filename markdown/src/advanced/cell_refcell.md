...
Rust 通过严格的所有权和借用规则保证安全性，但有时会限制灵活性。因此，`Cell` 和 `RefCell` 提供了**内部可变性**，让不可变引用的数据也能修改。
...
> 注意：`borrow_mut()` 和 `borrow()` 不能同时存在，否则运行时 panic：
>
> ```rust
...
- `Rc<T>` → 多个所有者
- `RefCell<T>` → 内部可变性
- 缺点：`borrow_mut()` 仍需小心，违反规则则 panic。
