...
Option 和 Result 的组合子（combinator）让错误处理可以写成流式链式调用，避免大量嵌套的 match/if let。

核心思路：把“可能没有值”或“可能出错”的操作串联起来，只在最终一次解包，中间全部用组合子传递。
...
> ⚠️ TS 没有这些是因为 null/undefined 没有“包装器”。Rust 的 Option/Result 是真正的类型，编译器强制你处理。
