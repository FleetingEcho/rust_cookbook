...
Cargo features 让一个 crate 可以有可选功能，使用方按需开启，避免引入不需要的依赖或代码。
...
以下 `"logging"` 是演示用的虚构 feature，真实项目里需要在 Cargo.toml `[features]` 里声明。这里用 `allow(unexpected_cfgs)` 消除警告，表明这是教学示例的有意为之。
...
最常见的 cfg 用法：只在 `cargo test` 时编译测试代码。每个测试模块都有 `#[cfg(test)]`。
